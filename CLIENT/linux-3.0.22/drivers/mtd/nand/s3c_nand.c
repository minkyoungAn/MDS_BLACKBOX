...
#if defined(CONFIG_CPU_S3C2450) || defined(CONFIG_CPU_S3C2416) || defined(CONFIG_CPU_S3C2412)
				     ...
				     u_char *read_ecc, u_char *calc_ecc)
{
	...
	if (!dat) {
		...
		case 1: /* 1 bit error (Correctable)
			...
			pr_debug("s3c-nand: 1 bit error detected at byte %ld, correcting from "
					"0x%02x ", (nfestat0 >> 7) & 0x7ff, dat[(nfestat0 >> 7) & 0x7ff]);
			dat[(nfestat0 >> 7) & 0x7ff] ^= (1 << ((nfestat0 >> 4) & 0x7));
			pr_debug("to 0x%02x...OK\n", dat[(nfestat0 >> 7) & 0x7ff]);
			ret = 1;
			break;

		case 2: /* Multiple error */
		case 3: /* ECC area error */
			pr_debug("s3c-nand: ECC uncorrectable error detected\n");
			ret = -1;
			break;
		}
...
#if defined(CONFIG_CPU_S3C2450) || defined(CONFIG_CPU_S3C2416) || defined(CONFIG_CPU_S3C2412)
		writel(0xc5, info->regs + S3C2440_NFCONT);
#endif
