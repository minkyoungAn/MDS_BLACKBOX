...
static int s3c_fb_set_par(struct fb_info *info)
{
	...
	if (win_no == sfb->pdata->default_win) {
		...
		// Gemini 2014.02.08 from mango
//		data |= VIDCON0_CLKSEL_HCLK; 
		data |= VIDCON0_CLKSEL_LCD;
		data &= ~VIDCON0_CLKDIR;

		writel(data, regs + VIDCON0);

		data = readl(S3C2443_CLKDIV1);
		data |= ((var->pixclock/1000) << 16) ; //DISPDIV 1=48Mhz 2=33Mhz 3=24Mhz		Gemini 2014.02.08
		writel(data, S3C2443_CLKDIV1);

		//temp = readl(regs + VIDCON0);
