...
static irqreturn_t smsc911x_irqhandler(int irq, void *dev_id)
{
	...
	if (unlikely(intsts & inten & INT_STS_SW_INT_)) {
        // printk("Line(%d) intsts: %08X, inten: %08X\n", __LINE__, intsts, inten);
		temp = smsc911x_reg_read(pdata, INT_EN);
		temp &= (~INT_EN_SW_INT_EN_);
		smsc911x_reg_write(pdata, INT_EN, temp);
    ...
    if(IRQ_HANDLED != serviced)
    {
        // printk("IRQ is not handled !!, intsts: %08X, inten: %08X\n", intsts, inten);
        /* IRQ 57 is only used in smsc911x, serviced value should be set with handled */
		serviced = IRQ_HANDLED;
    }
...
static int __devinit smsc911x_drv_probe(struct platform_device *pdev)
{
	...
	memcpy(dev->dev_addr, config->dev_addr, 6);
